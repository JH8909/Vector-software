...
const IN = 'E:/Claude/鐭㈤噺杞欢/samples/test-run';
const OUT = 'E:/Claude/鐭㈤噺杞欢/samples/test-out';
...
function extractVividSeeds(pixels, maxSeeds) {
  ...
  const list = [...buckets.values()].map(b => {
    ...
    return { c, n: b.n, ch: chroma(c) };
  }).filter(x => x.n >= 1 && x.ch >= 24).sort((a, b) => b.ch - a.ch || b.n - a.n);
  const seeds = [];
  for (const item of list) {
    if (seeds.length >= maxSeeds) break;
    if (seeds.some(s => deltaE(s, item.c) < 22)) continue;
    seeds.push(item.c);
  }
  return seeds;
}
function pickPalette(candidates, pixels, limit) {
  ...
  for (const px of pixels) {
    ...
    for (let j = 0; j < candidates.length; j++) { const d = deltaE(px, candidates[j]); if (d < md) { md = d; best = j; } }
    counts[best]++;
  }
  const ranked = candidates.map((c, i) => ({ c, n: counts[i], ch: chroma(c) })).filter(x => x.n > 0).sort((a, b) => b.n - a.n || b.ch - a.ch);
  const threshold = limit <= 2 ? 30 : limit <= 4 ? 24 : 20;
  const selected = [];
  for (const item of ranked) {
    if (selected.length >= limit) break;
    if (selected.some(s => deltaE(item.c, s) < threshold)) continue;
    selected.push(item.c);
  }
  const vivid = [...ranked].filter(x => x.ch >= 28).sort((a, b) => b.ch - a.ch || b.n - a.n);
  for (const item of vivid) {
    if (selected.length >= limit) {
      const replaceIdx = selected.findIndex(s => chroma(s) < 18);
      if (replaceIdx < 0) break;
      if (selected.some(s => deltaE(item.c, s) < threshold * 0.75)) continue;
      if (item.n < Math.max(2, pixels.length * 0.00015)) continue;
      selected[replaceIdx] = item.c;
      continue;
    }
    if (selected.some(s => deltaE(item.c, s) < threshold * 0.75)) continue;
    if (item.n < Math.max(2, pixels.length * 0.00015)) continue;
...
function pickDom(pxs) {
  ...
  for (const p of pxs) {
    ...
    const b = buckets.get(key);
    if (b) b.n++; else buckets.set(key, { c: p, n: 1 });
  }
  ...
  if (bestScore < 0) {
    let darkest = pxs[0], minLum = 999;
    for (const p of pxs) { const lum = p[0] + p[1] + p[2]; if (lum < minLum) { minLum = lum; darkest = p; } }
    return darkest;
  }
  return best;
}
...
function softFringe(data, w, h) {
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    const t = a / 255;
    const cr = Math.round(data[i] * t + bg[0] * (1 - t)), cg = Math.round(data[i + 1] * t + bg[1] * (1 - t)), cb = Math.round(data[i + 2] * t + bg[2] * (1 - t));
    if (Math.hypot(cr - bg[0], cg - bg[1], cb - bg[2]) < 18) { out[i] = bg[0]; out[i + 1] = bg[1]; out[i + 2] = bg[2]; out[i + 3] = 0; }
...
function denoise(mask, w, h, minN) {
  ...
  for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) {
    ...
    for (let dy = -1; dy <= 1; dy++) for (let dx = -1; dx <= 1; dx++) {
      ...
      if (nx < 0 || nx >= w || ny < 0 || ny >= h) continue; if (mask[ny * w + nx] === 1) nbs++;
    }
...
function absorb(layers, w, h, totalPx) {
  ...
  for (let i = result.length - 1; i >= 0; i--) {
    ...
    if (chroma(layer.color) >= 35) continue;
    ...
    for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) {
      ...
      for (let dy = -1; dy <= 1; dy++) for (let dx = -1; dx <= 1; dx++) {
        if (!dx && !dy) continue; const nx = x + dx, ny = y + dy;
        if (nx < 0 || nx >= w || ny < 0 || ny >= h) continue; const ni = ny * w + nx;
    ...
    const mostlyBoundary = bestTouch >= layer.pixelCount * 0.55;
...
function buildLayers(data, w, h, palette, noise) {
  const pc = w * h; const assign = new Uint8Array(pc);
  const accum = palette.map(() => ({ r: 0, g: 0, b: 0, count: 0 }));
  ...
  for (let i = 0; i < pc; i++) {
    ...
    if (single) {
      if (deltaE(px, bg) < 28) { assign[i] = 255; continue; }
      if (deltaE(px, palette[0]) > 55) { assign[i] = 255; continue; }
      assign[i] = 0; accum[0].r += px[0]; accum[0].g += px[1]; accum[0].b += px[2]; accum[0].count++; continue;
    }
    let md = Infinity, best = 0;
    for (let j = 0; j < palette.length; j++) { const d = deltaE(px, palette[j]); if (d < md) { md = d; best = j; } }
  ...
  for (let i = 0; i < pc; i++) {
    ...
    if (single) {
      if (deltaE(px, bg) < 28) continue;
      if (deltaE(px, pal[0]) > 55) continue;
      layers[0].mask[i] = 1; layers[0].pixelCount++; continue;
    }
    let md = Infinity, best = 0;
    for (let j = 0; j < pal.length; j++) { if (!accum[j].count) continue; const d = deltaE(px, pal[j]); if (d < md) { md = d; best = j; } }
...
function cc(mask, w, h, minPx) {
  const visited = new Uint8Array(mask.length); const comps = [];
  ...
  for (let i = 0; i < mask.length; i++) {
    ...
    while (head < tail) {
      const ci = q[head++]; comp[ci] = 1; const cy = Math.floor(ci / w), cx = ci % w;
      for (const [dx, dy] of D) {
        const nx = cx + dx, ny = cy + dy; if (nx < 0 || nx >= w || ny < 0 || ny >= h) continue;
        const ni = ny * w + nx; if (mask[ni] === 1 && !visited[ni]) { visited[ni] = 1; q[tail++] = ni; }
      }
    }
...
function split(layers, w, h, minPx) {
  ...
  for (const layer of layers) {
    ...
    if (isNearGray && chroma(layer.color) < 22 && (isLight || layer.pixelCount < total * 0.08)) continue;
...
function maskRgba(mask, w, h) {
  const buf = Buffer.alloc(w * h * 4);
  for (let i = 0; i < mask.length; i++) { const oi = i << 2; const v = mask[i] === 1 ? 0 : 255; buf[oi] = buf[oi + 1] = buf[oi + 2] = v; buf[oi + 3] = 255; }
  return buf;
}
...
async function run(file, settings) {
  ...
  if (Math.max(srcW, srcH) < 1500) {
    const r = 1500 / Math.max(srcW, srcH);
    img = img.resize(Math.round(srcW * r), Math.round(srcH * r), Jimp.RESIZE_BICUBIC);
  }
  const w = img.bitmap.width, h = img.bitmap.height;
  const data = softFringe(Buffer.from(img.bitmap.data), w, h);
  ...
  for (const comp of comps) {
    let rgba = maskRgba(comp.mask, w, h);
    if (comps.length >= 2) rgba = dilate(rgba, w, h);
    try {
      ...
      if (paths.length) groups.push({ fill: comp.fill, paths, pixelCount: comp.pixelCount, color: comp.color });
    } catch {}
  }
...
(async () => {
  ...
  for (const [f, s] of cases) {
    ...
    console.log(JSON.stringify({ file: r.file, layers: r.layers, fills: r.fills.join('|'), seeds: r.seeds.join('|') }));
  }
})();

/**
 * Node 侧回归测试：用 sharp 读图 + 复刻浏览器管线关键步骤 + potrace
 * 输出 SVG 到 samples/test-out，并打印质量指标
 */
const fs = require('fs');
...
function mergeSimilar(palette, limit) {
  ...
  if (unique.length <= 1) return unique.length ? unique : [[0, 0, 0]];
...
function noiseToMinNeighbors(noiseReduction) {
...
function absorbFringe(layers, width, height, totalPx) {
  ...
  const result = sorted.map(l => ({ ...l, mask: new Uint8Array(l.mask) }));
  ...
  for (let i = result.length - 1; i >= 0; i--) {
    ...
    for (let p = 0; p < layer.mask.length; p++) {
      if (layer.mask[p] === 1) { target.mask[p] = 1; layer.mask[p] = 0; }
    }
...
function buildColorLayers(data, width, height, samplePalette, noiseReduction) {
  ...
  for (let i = 0; i < pc; i++) {
    ...
    assign[i] = best;
    accum[best].r += px[0]; accum[best].g += px[1]; accum[best].b += px[2]; accum[best].count++;
  }

  const palette = accum.map(a => a.count > 0
    ? [Math.round(a.r / a.count), Math.round(a.g / a.count), Math.round(a.b / a.count)]
    : [0, 0, 0]);

  const layers = palette.map(color => ({
    color, fill: toHex(...color), mask: new Uint8Array(pc), pixelCount: 0,
  }));
  ...
  return absorbFringe(kept, width, height, pc);
}
...
function splitAndMerge(layers, width, height, minPx) {
  ...
  for (const layer of layers) {
    ...
    for (const comp of comps) {
      for (let i = 0; i < comp.length; i++) if (comp[i] === 1) { merged[i] = 1; totalPc++; }
    }
...
function dilateMaskBuf(mask, width, height) {
...
function softFringe(data, width, height) {
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    const d = Math.hypot(cr - bg[0], cg - bg[1], cb - bg[2]);
    if (d < 18) {
      out[i] = bg[0]; out[i + 1] = bg[1]; out[i + 2] = bg[2]; out[i + 3] = 0;
    } else {
...
async function loadImage(filePath) {
  ...
  return { data: Buffer.from(data), width: info.width, height: info.height, srcW: meta.width, srcH: meta.height };
}
...
async function vectorizeFile(filePath, settings) {
  ...
  if (n <= 1 && layers.length > 1) {
    layers = [layers.reduce((a, b) => a.pixelCount >= b.pixelCount ? a : b)];
  }
  ...
  for (let idx = 0; idx < comps.length; idx++) {
    ...
    if (comps.length >= 2) rgba = dilateMaskBuf(rgba, width, height);
    try {
      ...
      groups.push({ fill: comp.fill, paths, pixelCount: comp.pixelCount });
    } catch (e) {
      console.warn('  skip layer', comp.fill, e.message);
    }
  }

  const gXml = groups.map((g, i) => {
    const id = `layer_${String(i + 1).padStart(3, '0')}`;
    const ds = g.paths.map(p => (p.match(/d="([^"]*)"/) || [])[1]).filter(Boolean);
    const d = ds.join(' ');
    return `  <g id="${id}" data-fill="${g.fill}"><path fill="${g.fill}" fill-rule="evenodd" d="${d}"/></g>`;
  }).join('\n');
...
async function main() {
  ...
  for (const file of files) {
    const fp = path.join(IN_DIR, file);
    const t0 = Date.now();
    try {
      // 按估计色数选模式倾向
      const preview = await sharp(fp).ensureAlpha().raw().toBuffer({ resolveWithObject: true });
      const est = estimateColors(preview.data);
      ...
      const result = await vectorizeFile(fp, settings);
      const outName = file.replace(/\.[^.]+$/, '') + '.svg';
      fs.writeFileSync(path.join(OUT_DIR, outName), result.svg);
      const row = {
        file,
        src: `${result.srcW}x${result.srcH}`,
        used: `${result.width}x${result.height}`,
        est: result.estColors,
        req: result.requested,
        layers: result.layers,
        fills: result.fills.join(','),
